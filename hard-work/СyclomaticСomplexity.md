...
Тогда его автомат состояний будет представлять классы с именем: *DeleteClipperTool*, *AddClipperTool*.  
Тобишь в имени класса отразили какое состояние он представляет.  
Круто в этом методе то, что мы можем избавиться от *ACTIVE* и *NULL*, потому что *AddClipperTool* уже по умолчанию *ACTIVE* и не *NULL*. Сразу убираем 2 if-а. Аналогично для *DeleteClipperTool*.
